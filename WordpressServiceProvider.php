...
class WordpressServiceProvider extends ServiceProvider {
	...
	// builds heirarchical menu object based off of the WordPress Appearance->Menu name strings
	// if you pass the current page's post ID, that element will be flagged as active.
	...
	// function to determine if URL exists, and if so, what type of post it is
		...
		if (!apc_exists('check_route_' . $url)) {
			...
			if (count($query->posts) > 0) {
				...
				if ('page' == $query->posts[0]->post_type) {
					$post->type = 'page';
					// return post type as page
				} else {
					$post->type = 'post';
					// return post type as post
				}
			}
	...
	// This function merely obfuscates the WP_Query method.
	// It will build a query and get all post objects that meet the query parameters.
